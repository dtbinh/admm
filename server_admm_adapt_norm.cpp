...
void initialize_all_addresses(vector<attributes>& all_neighbors_attr)
{
  ...
  initialize_address_attr("172.16.0.6", "172.16.0.5", all_neighbors_attr[0], "147.72.248.13", 0, 1, 1, 1);
...
void create_send_threads(vector<attributes>& all_neighbors_attr, vector<thread>& send_msg_cl_thread, vector<char_16_array>& buffer_send_vec, vector<int>& sockets_client, x_and_u& msg_send_serv, vector<double>& u_b)
{
  ...
  { 
    if (all_neighbors_attr[i].bridge == 1 && all_neighbors_attr[i].active == 1)
    {
      ...
      send_msg_cl_thread.push_back( thread(send_msg_struct_cl, sockets_client[i], count, &buffer_send_vec[i].buffer[0], msg_send_serv, len_send_serv) );
      count = count + 1;   
    }
  }
}
